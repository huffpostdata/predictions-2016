...
##      FOR NATIONAL MODEL, SELECT POLLSTERS 

#Read in and match pollsters to exclusion list
exclude <- read.csv("exclusionlist.csv")

#Good defined by exclusion list
match <- match(data$pollster,exclude$exclude, nomatch = 0)
data$goodlist <- ifelse(match==0, 1, 0)

#Good defined by inclusion list
match2 <- match(data$pollster,exclude$include, nomatch = 0)
data$goodlist2 <- ifelse(match2 !=0, 1, 0)

##subset of polls in each group
theGoodPolls <- ifelse(data$goodlist == 1, data$pollster, NA)
theGoodPollslist <- sort(unique(theGoodPolls))

theGoodPolls2 <- ifelse(data$goodlist2 == 1, data$pollster, NA)
theGoodPolls2list <- sort(unique(theGoodPolls2))

##subset data to only include goodlist based on inclusions
data <- data[data$goodlist2 == 1, ] 
